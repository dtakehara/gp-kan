"""
GP Models Comparison (Quick Version): GPKAN vs DeepGP vs SparseGP

f1-f5の関数について、3つのGPモデルのfitting性能を短時間で比較します。
エポック数とサンプル数を削減した高速版です。
"""
...
class GPModelComparison:
    ...
    def train_gpkan(self, hidden_size: int = 5, num_inducing: int = 10):
        """GPKANの訓練"""
        print(f"\n  Training GPKAN...")
    ...
    def train_sparse_gp(self, num_inducing: int = 30):
        """SparseGPの訓練"""
        print(f"\n  Training SparseGP...")
    ...
    def train_deep_gp(self, hidden_dims: list = [5], num_inducing: int = 20):
        """DeepGPの訓練"""
        print(f"\n  Training DeepGP...")
...
def main():
    ...
    print("=" * 80)
    print("GP Models Comparison (Quick Version): GPKAN vs DeepGP vs SparseGP")
    print("=" * 80)
    ...
    # 結果をCSVに保存
    csv_filename = f"gp_comparison_results_quick_{timestamp}.csv"
    ...
    # テキスト形式でも保存
    txt_filename = f"gp_comparison_results_quick_{timestamp}.txt"
    with open(txt_filename, "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("GP Models Comparison Results (Quick Version)\n")
